...
ggplot(MSIBU[!is.na(MSIBU$IBU),], aes(State, IBU ) ) +
...
ggplot(MSABV[!is.na(MSABV$ABV),], aes( State, ABV )) +
...
# Can't Figure out how to get to a dataframe and present well
...
# 7a.	Fun Chart
# write.csv(BeerBrew, file="BeerBrew.csv", row.names = FALSE)
f <- list(
...
pIBU <- plot_ly(BeerBrew, x = ~BeerBrew$IBU, color = BeerBrew$State, type = "box") %>% layout(xaxis = x, yaxis = y)
pIBU
...
*  YouTube Video - https://studio.youtube.com/#/channel/UCOUGjD8v1sf84pBwjLyBn7Q
